...
export class BootstrapService implements OnApplicationBootstrap {
  ...
  async onApplicationBootstrap(): Promise<void> {
    ...
    try {
      ...
      const sourceHash =
        await this.restaurantImportService.getFileHash(csvPath);
      const alreadyDone =
        await this.restaurantImportService.hasSuccessfulImport(
          sourceName,
          sourceHash,
        );
      if (!alreadyDone) {
        this.logger.log("Launching restaurant bootstrap import...");
        await this.restaurantImportService.runImport(csvPath, {
          strictCuisine: false,
        });
      } else {
        this.logger.log(
          "Restaurant import skipped: same file hash already imported successfully.",
        );
      }
  ...
  private getFatalStartupMessage(error: unknown): string | null {
    ...
    if (
      code === "42P01" ||
      err?.message?.includes('relation "ingestion_logs" does not exist') ===
        true
    ) {
