...
import {
  PaginatedQueryDto,
  SortDirection,
} from "src/_utils/dto/requests/paginated-query.dto";
...
export class SearchRestaurantsQueryDto extends PaginatedQueryDto {
  ...
  pageSize = 20;
  @ApiPropertyOptional({
    description: "Text search on restaurant name and description.",
  })
  @IsString()
  @Optional()
  search?: string;
  ...
  @ApiPropertyOptional({
    type: "integer",
    minimum: 1,
    description: "Filter by country id (exact).",
  })
  @Type(() => Number)
  @IsInt()
  @Min(1)
  ...
  @ApiPropertyOptional({
    type: "integer",
    minimum: 1,
    description: "Filter by city id (exact).",
  })
  @Type(() => Number)
  @IsInt()
  @Min(1)
  ...
  @ApiPropertyOptional({
    type: "array",
    items: { type: "integer" },
    description: "Cuisine ids (any match), comma-separated or repeated.",
  })
  @Transform(toIdArray)
  @IsArray()
  @IsInt({ each: true })
  ...
  @ApiPropertyOptional({
    type: "array",
    items: { type: "integer" },
    description: "Facility ids (any match), comma-separated or repeated.",
  })
  @Transform(toIdArray)
  @IsArray()
  @IsInt({ each: true })
  ...
  @ApiPropertyOptional({
    enum: RestaurantSearchSortBy,
    default: RestaurantSearchSortBy.NAME,
    description: "Restaurants list sort key.",
  })
